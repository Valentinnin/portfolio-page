function getTimeInWords() {
    ...
    if (minute == 0) {
        ...
        output += minuten[30 - minute] + " vor halbi " + stunden[(stunde + 1) % 12];
    } else if (minute == 30) {
        output += "halbi " + stunden[(stunde + 1) % 12];
    } else if (minute < 40) {
        output += minuten[minute - 30] + " ab halbi " + stunden[(stunde + 1) % 12];
    } else {
...
setInterval(getTimeInWords, 60000); // 60000 Millisekunden für eine Minute
